...
async def list_sla(db: AsyncSession = Depends(get_db)):
...
async def update_sla(priority: str, payload: SLAConfigUpdate, db: AsyncSession = Depends(get_db)):
    await _ensure_defaults(db)
...
async def approve_sla(priority: str, payload: SLAApprove, db: AsyncSession = Depends(get_db)):
    """HIL-1: Approve SLA config before AI agents use it."""
    result = await db.execute(select(SLAConfig).where(SLAConfig.priority == priority.upper()))
    config = result.scalar_one_or_none()
    if not config:
